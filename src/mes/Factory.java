...
public class Factory {
    ...
    public boolean initFactory()
    {
        ...
        if(this.addConveyors("transport", "linear", 16))
        {
            generateTransportConveyorID();
            generateTransportMemoryIndexes();
            //Initializing Block Array, needs some testing
            blocksInFactory = new ArrayList<>();
            return true;
        }
        // no error ocurred
        else
        {
            System.out.println("Error adding conveyors.\n");
            return false;
        }
    ...
    {
        // if some error ocurred reading sensors
        if (!factoryMonitor.readSensors())
        {
            System.out.println("Factory sensors were not read.\n");
            return false;
        }
        // if some error ocurred reading actuators
        else if (!factoryMonitor.readActuators())
        {
            System.out.println("Factory actuators were not read.\n");
            return false;
        }
        // if no error ocurred reading factory
        else
        {
            // updates factoryData
